    ...
    render::RenderBuffer& buf)
{
    ...
    for(int py=0; py<height; py++){
        for(int px=0; px<width; px++){
            int buf_depth_idx = get_buf_index(buf, "depth", py, px);
            if(buf.depth[buf_depth_idx]>0.8) valid_pix++;
        }
    }
    if(valid_pix/(height*width) > 0.8)
        buf.is_valid = true;
...
void render::dump_buffer(render::RenderBuffer& buf, const std::string& scene_dir, const std::string& fname){
    ...
    xt::xarray<unsigned char> depth_frame_png = depth_frame_npz;
    ...
    xt::xarray<unsigned char> normal_frame_png = 255*(normal_frame_npz+1)*0.5;
