...
namespace render{
    ...
    typedef struct RenderBuffer{
        ...
        RenderBuffer(int w, int h):width(w), height(h) {
            ...
            depth_vis = (unsigned char*)malloc(width*height*sizeof(unsigned char));
            color = (unsigned char*)malloc(width*height*3*sizeof(unsigned char));
            normal = (double*)malloc(width*height*3*sizeof(double));
            normal_vis = (unsigned char*)malloc(width*height*3*sizeof(unsigned char));
            ...
            for(int h = 0; h < height; ++h){
                for(int w = 0; w < width; ++w){
                    ...
                    // pixmeta 
                    int buf_pm_idx = width*12*h + w*12;
                    for(int i=0; i<12; i++)
        ...
        double* depth;
        unsigned char* depth_vis;
        unsigned char* color;
        double* normal;
        unsigned char* normal_vis;
        double* pixcoord;
        double* pixmeta;
        int* mesh_vertex;
        bool is_valid;
    ...
    void dump_buffer(render::RenderBuffer& buf, const std::string& scene_dir, const std::string& prefix, const std::string& fname);
}
