...
# Point cloud IO
# ----------------------------------------
def read_ply_vertices_faces(filename):
    ...
    face_array = np.array([[f[0][0], f[0][1], f[0][2]] for f in face])
    return pc_array, face_array
...
def write_ply_vertices_faces(points, colors, faces, filename, text=True):
    ...
    colors = [(colors[i,0], colors[i,1], colors[i,2], colors[i,3]) for i in range(colors.shape[0])]
    color = np.array(colors, dtype=[('red', 'u1'), ('green', 'u1'),('blue', 'u1'), ('alpha', 'u1')])
    ...
    face_el = PlyElement.describe(faces_array, 'face')
    PlyData([el, face_el], text=text).write(filename)
