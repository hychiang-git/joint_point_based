...
def get_list(scene_dir, color_dir, depth_dir=None, normal_dir=None, pixcoord_dir=None, pixmeta_dir=None, pose_dir=None, label_dir=None, \
    ...
    if depth_dir is not None:
        ...
        assert len(depth_imgs) == len(color_imgs),\
           '[Error] {}, Image length not equal: depth {}, color {}' \
           .format(scene_dir, len(depth_imgs), len(color_imgs))
    if normal_dir is not None:
        ...
        assert len(normal_imgs) == len(color_imgs),\
           '[Error] {}, Image length not equal: normal {}, color {}' \
           .format(scene_dir, len(normal_imgs), len(color_imgs))
    ...
    if pose_dir is not None:
        ...
        assert len(pose_txts) == len(color_imgs),\
           '[Error] {}, Image length not equal: pose {}, color {}' \
           .format(scene_dir, len(pose_txts), len(color_imgs))
...
def get_test_image_sample(image_path, depth_path, get_depth=True):
    # Get a random image, label and camera pose from scene
    color_img = Image.open(image_path)  # load color
    color_img = np.array(color_img)
    if get_depth:
        ...
        depth_img = np.zeros(color_img.shape)
    assert color_img.shape[0:2] == depth_img.shape[0:2],\
           'height, width not equal corlor {}, depth {}'\
           .format(color_img.shape, depth_img.shape)
    height = color_img.shape[0]
    width = color_img.shape[1]
    return color_img, depth_img

def get_test_depth_coord_sample(image_path, depth_path, int_param, rotation, translation):
    color_img = Image.open(image_path)  # load color
    color_img = np.array(color_img)
    # detph to camera xyz 
    depth_img = np.load(depth_path)['depth']  # load depth
    ...
    coord_img = coord_img.reshape(height, width, 3)
    coord_img = np.concatenate([coord_img, color_img], axis=-1)
    return coord_img, valid_idx 

def get_test_coord_sample(image_path, pixcoord_path, int_param, rotation, translation):
    color_img = Image.open(image_path)  # load color
    color_img = np.array(color_img)
    # detph to camera xyz 
    pixcoord_img = np.load(pixcoord_path)['pixcoord']  # load depth
    assert color_img.shape[0:2] == pixcoord_img.shape[0:2],\
           'height, width not equal corlor {}, pixel coord {}'\
           .format(color_img.shape, pixcoord_img.shape)
    height = pixcoord_img.shape[0]
    width = pixcoord_img.shape[1]
    #pixel_z = np.array(depth_img) / 1000.0
    valid_idx = np.sum(pixcoord_img != np.array([0,0,0]), axis=-1) > 0
...
def get_test_scene_data(cam_int_param, get_depth, get_coord, get_pixmeta, point_from_depth, data_dir):
    color_dir, depth_dir, normal_dir, pixcoord_dir, pixmeta_dir, pose_dir = data_dir
    #print(color_dir, pixmeta_dir)
    color_img, depth_img = \
        get_test_image_sample(color_dir, depth_dir, get_depth)
    #print(color_img.shape, depth_img.shape, pixel_label.shape, pixel_weight.shape)
    if get_coord is True:
        ext_param = np.loadtxt(pose_dir)
        if np.any(np.isnan(ext_param)) or np.any(np.isinf(ext_param)):
        ...
        rotation = ext_param[0:3, 0:3]
        if point_from_depth is True:
            coord_img, valid_coord_idx = \
                get_test_depth_coord_sample( \
                                        color_dir, depth_dir,\
                                        cam_int_param, rotation, translation)
        else: # load from pixcoord
            coord_img, valid_coord_idx = \
                get_test_coord_sample(\
                                  color_dir, pixcoord_dir, \
                                  cam_int_param, rotation, translation)
    else:
        coord_img = np.zeros((color_img.shape[0], color_img.shape[1], 6))
        valid_coord_idx = np.zeros((color_img.shape[0], color_img.shape[1]))
    if get_pixmeta is True:
        pixel_world_coord = np.load(pixcoord_dir)['pixcoord']  # load depth
        pixel_meta = np.load(pixmeta_dir)['meta']
        pixel_meta = np.concatenate([pixel_world_coord, pixel_meta], axis=-1)
