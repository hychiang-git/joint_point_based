...
def dump_point_cloud(scene_name, output_dir, points, label=None, weight=None, vis_weight_factor=40):
    ## dump color point clouds
    coord = points[:, 0:3]
    color = points[:, 3:6]
    pc_util.write_ply_rgb(coord, color, os.path.join(output_dir, scene_name+'_points.ply'))
    if label is not None:
        ...
        pc_util.write_ply_color(coord, label_nyu, os.path.join(output_dir, scene_name+'_label.obj'), len(color_map), color_map)
    if weight is not None:
       # dump weight 
        weight_color = vis_weight_factor * np.expand_dims(weight, axis=-1).astype(np.uint8)
        weight_color = np.concatenate((weight_color, weight_color, weight_color), axis=-1)
        pc_util.write_ply_rgb(coord, weight_color, os.path.join(output_dir, scene_name+'_weight.ply'))


def dump_point_cloud_from_coord(scene_name, output_dir, coord_img, valid_coord_idx, coord_label=None, coord_weight=None, vis_weight_factor=40):
    ## dump color point clouds
